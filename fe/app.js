        ...
        const translations = {
            vi: {
                ...
                mainSubtitle: '<strong>Tìm đúng dữ liệu. Thử miễn phí. Kết nối vào công việc.</strong> Kiểm tra nguồn, phạm vi lịch sử và dữ liệu mẫu trước khi tải file hoặc dùng Google Sheets / API.',
                dataHeroCTADocs: 'Dùng trong Google Sheets',
                dataHeroCTAPaid: 'API',
                ...
                aboutP2: '<strong>Giá vàng miếng SJC</strong> — giá mua vào/bán ra do SJC công bố, cập nhật 2 tiếng/lần trong giờ hành chính. <strong>Giá bạc trong nước</strong> <strong>Lãi suất liên ngân hàng</strong> do Ngân hàng Nhà nước Việt Nam (SBV) công bố. <strong>Lãi suất gửi tiết kiệm</strong> từ các ngân hàng thương mại lớn.',
            ...
            en: {
                ...
                mainSubtitle: '<strong>Find the right data. Try it free. Put it to work.</strong> Inspect sources, history coverage and sample records before downloading or connecting Google Sheets / API.',
                dataHeroCTADocs: 'Use in Google Sheets',
                dataHeroCTAPaid: 'API',
                ...
                downloadSubtitle: 'Download SJC bar gold prices, silver prices and bank interest rates for free, via CSV or a RESTful API.',
                goldDatasetTitle: 'Vietnam Historical Gold Price',
                goldDatasetDesc: 'Download Vietnam SJC bar gold price history for free. Buy/sell quotes refreshed every 2 hours during office hours.',
                ...
                aboutP2: '<strong>SJC bar gold</strong> — buy/sell quotes published by SJC, refreshed every 2 hours during Vietnamese office hours. <strong>Domestic silver prices</strong>. <strong>Interbank rates</strong> published by the State Bank of Vietnam (SBV). <strong>Term deposit rates</strong> from major commercial banks.',
        ...
        function loadChartsForSection(sectionKey) {
            ...
            if (sectionKey === 'gold-silver') {
                const goldType = document.getElementById('goldTypeSelect')?.value || 'SJC';
                // The LBMA forecast line is drawn inside loadChartData('gold', ...)
                // itself now (fetchLbmaSurvey(), called from there) — not a
                // separate load here.
                Promise.all([
        ...
        // Best-effort fetch of the LBMA gold forecast survey (aggregate only —
        // see CLAUDE.md's "LBMA gold forecast survey" section). Small,
        // irregular-cadence file (2 reports/year), fetched once and cached —
        // there is no "period" to key it by, unlike fetchWorldOverlay above.
        let _lbmaSurveyCache = null;
        async function fetchLbmaSurvey() {
            if (_lbmaSurveyCache !== null) return _lbmaSurveyCache;
            try {
                const res = await fetch('./data/lbma_gold_survey.json');
                if (res.ok) {
                    _lbmaSurveyCache = await res.json();
                    return _lbmaSurveyCache;
                }
            } catch (_) { /* best-effort — domestic chart renders without it */ }
            return null;
        }
        ...
        ========================================================= */
        function renderChart(chartType, apiData, period, worldData, lbmaData) {
            ...
            if (chartType === 'gold' || chartType === 'silver') {
                chartData = parseGoldSilverData(apiData, chartType, worldData, lbmaData);
        ...
        function parseGoldSilverData(apiData, chartType, worldData, lbmaData) {
            ...
            // LBMA gold forecast survey — a flat dashed reference line on the
            // SAME world axis, at the latest report's average. Same idea as the
            // SBV policy-rate lines overlaid on the interbank chart (CLAUDE.md's
            // chart-honesty rules): a reference level plotted alongside the real
            // series so the reader can see how far price sits from what
