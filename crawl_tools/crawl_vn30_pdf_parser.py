...
Reads PDF URLs from vn30_pdf_index, OCRs with tesseract, extracts key financial figures,
and upserts into vn30_income_stmt_quarterly / vn30_balance_sheet_quarterly / vn30_cashflow_quarterly
...
def ocr_pdf(pdf_bytes: bytes, dpi: int = 220) -> list[str]:
...
def extract_income_stmt(page_texts: list[str]) -> dict:
    """Extract key income statement figures from OCR'd text.

    Search strategy: IS-classified pages first, then all pages as fallback.
    This prevents picking up subsidiary figures from non-IS pages (e.g. BVH
    insurance subsidiaries whose 'thu phi bao hiem goc' appears before the
    consolidated IS section).
    """
    is_lines = []
    for text in page_texts:
    ...
    # Use IS-only lines when available, fall back to all lines
    search_lines = is_lines if is_lines else all_lines

    # Revenue — industrial / banking / insurance variants
    revenue = (
    ...
    return {
        ...
        'eps':          eps,    # VND/share — no unit conversion
    }


def extract_cashflow(page_texts: list[str]) -> dict:
    """Extract key cash flow figures from OCR'd text."""
    cf_lines = []
    for text in page_texts:
...
def parse_pdf(pdf_id: int, ticker: str, year: int, quarter: int | None, pdf_url: str,
    ...
    if verbose:
        print(f"    OCR done: {len(page_texts)} pages")

    # Extract
    try:
        bs = extract_balance_sheet(page_texts)
        inc = extract_income_stmt(page_texts)
        cf = extract_cashflow(page_texts)
    except Exception as e:
