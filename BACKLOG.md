...
- [x] Excel Add-in — manifest.xml + task pane, mount /excel-addin/, CORS for Office domains; workbook `.xlsx` tải từ `/api/v1/excel/workbook` có 9 Excel Table và refresh tại chỗ qua Office.js trên Mac/Windows/web bằng một request `/api/v1/excel/refresh-data` (không VBA, không lưu API key trong file)
