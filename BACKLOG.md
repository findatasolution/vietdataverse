...
- [ ] **Fuel-forecast B2B (dự báo giá xăng dầu trong nước) — Phase 1, Plan 1 (data pipeline) đang triển khai** trên branch `feat/fuel-forecast-pipeline`. Kiến trúc medallion (Bronze R2 → Silver `FUEL_FORECAST_DB`), model hybrid NĐ80, feature MRI, corporate API riêng OAuth2 + `.corp-prod-env`. **Plan 1 LIVE + verified** (2026-07-11): 15 unit test xanh; hạ tầng đã cấp (Neon `FUEL_FORECAST_DB` + R2 `vdv-raw`); backfill thật xong — **Silver: 23 cycle rows** (8 kỳ BCT, RON95/E5/DO) + **502 world rows** (Brent/RBOB 1 năm); **Bronze R2: 56 raw + meta sha256**. Parser đã robust (E10 rename, spike diesel ~205 USD/bbl, regex retail anchored). `.corp-prod-env` đã điền (600, gitignored). **Insight:** formula NĐ80 vs retail lệch tăng theo world → Quỹ BOG/miễn thuế damp giá khi spike (Plan 2 phải model). Tài liệu private `docs/research/`. **Còn lại:** merge branch `feat/fuel-forecast-pipeline` → main + set GitHub secrets (`FUEL_FORECAST_DB`, `R2_*`, `R2_BUCKET_RAW`) để bật weekly `fuel-pipeline.yml`. **Kế tiếp:** Plan 2 (model+backtest — đã có data thật), 3 (public API), 4 (corp service). **Trước khi thu tiền:** ý kiến luật sư NĐ169 + license nguồn futures.
