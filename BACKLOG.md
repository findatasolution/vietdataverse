...
- **Đã chốt (2026-09-08)**: `pe`/`pb`/`ps`/`dividend_yield`/`market_cap_billion` — bỏ hẳn khỏi mọi response API + FE, vĩnh viễn (không phải tạm ẩn). `roe`/`roa`/`eps` — chuyển sang tự tính từ BCTC sạch, không đọc `vn30_ratio_daily` nữa.
- Crawler vnstock3 vẫn chạy nhưng **chỉ nội bộ**, dùng đối chiếu số tự tính — không public. Có sunset: ngừng hẳn 2-4 quý sau khi pipeline BCTC tự tính ổn định. Xem rủi ro chưa-sạch-hoàn-toàn của cách này trong doc kế hoạch.
- Việc cần làm: (1) email tác giả vnstock xin phép bằng văn bản, (2) viết lại crawler BCTC lấy từ IR/cổng công bố thông tin chính thức thay vì vnstock3, (3) sửa `be/routers/vn30_data.py` bỏ field giá khỏi response.
- Kế hoạch đầy đủ: `docs/research/2026-09-08-financial-statements-subscription-plan.md`.
- **Tại sao HIGH:** gate bắt buộc trước khi bán gói BCTC, và là rủi ro pháp lý đang chạy live ngay cả ở bản miễn phí hiện tại.
...
- [ ] **Fuel-forecast B2B (dự báo giá xăng dầu trong nước) — Phase 1, Plan 1 (data pipeline) đang triển khai** trên branch `feat/fuel-forecast-pipeline`. Kiến trúc medallion (Bronze R2 → Silver `FUEL_FORECAST_DB`), model hybrid NĐ80, feature MRI, corporate API riêng OAuth2 + `.corp-prod-env`. **Plan 1 + Plan 2 LIVE** (merged PR #1 + Plan 2 on main, 2026-07-12). 75 unit test xanh. **Dữ liệu (cập nhật 2026-09-08):** 24–25 kỳ BCT/fuel (đến 2026-08-27, xem sự cố dưới), Brent/RBOB đến 2026-09-03 (1.082 rows), Bronze R2 raw + sha256. **Model (đột phá):** level model từ proxy Brent/RBOB **thua** random-walk (skill −0.10…0.00); **delta pass-through trên giá MOPS đúng loại (world_avg_price mà chính MOIT công bố trong mỗi bản tin, KHÔNG phải Brent/RBOB) thắng RW cả 3 fuel**: `retail_t ≈ retail_{t−1} + k·Δworld`. **Walk-forward (n=17–18, refresh 2026-09-08): MAE DO 446 / E5 607 / RON95 730 VND/L, coverage 88–94%, skill +0.50…+0.82** — con số bán B2B, ghi trong `fuel_backtest`. CI weekly `fuel-pipeline.yml` (extract-land ▶ model) xanh với đủ secrets.
  - **Sự cố đã fix (2026-09-08): pipeline BCT đứng im 2 tháng (09/07→27/08) trong khi CI báo xanh mỗi tuần.** MOIT chuyển bản tin "điều hành giá xăng dầu" từ category `thi-truong-trong-nuoc` sang `phat-trien-nang-luong` mà không báo; `discover_latest()` chỉ quét category cũ, cứ khớp lại đúng link 09/07 và upsert idempotent → exit 0 mỗi tuần dù không có dữ liệu mới. Đã backfill 2 kỳ thật (13/08, 27/08, xác minh bằng crawl trực tiếp) và sửa `crawl_moit_fuel.py`: quét cả 2 category + dò trực tiếp URL theo pattern cố định (không phụ thuộc category listing, vốn đã chứng minh không đáng tin — kỳ 27/08 không được link ở category nào cả); ngưỡng cảnh báo cứng 25 ngày không tiến kỳ mới → fail loud thay vì im lặng thành công. Chi tiết: `CLAUDE.md` mục "Fuel (domestic) crawl — MOIT re-files bulletins without notice". Nhịp kỳ giá có vẻ đã đổi từ hàng tuần sang ~2 tuần/lần kể từ 08/2026 (trùng thời điểm MOIT lấy ý kiến dự thảo Nghị định kinh doanh xăng dầu mới, 27/07/2026) — chưa chắc chắn 100%, cần theo dõi thêm.
  - **Business case đã lượng hoá:** proxy futures free KHÔNG đủ → license feed Platts/MOPS daily = chuyển skill âm thành dương → mục chi license là bắt buộc để bán forecast pre-announcement; không có feed thì bán scenario world-conditional. **Kế tiếp:** Plan 3 (public API giá lịch sử), Plan 4 (corporate API service + `.corp-prod-env`) — cả hai CHƯA bắt đầu, không có router fuel nào trong `be/main.py`. **Trước khi thu tiền:** ý kiến luật sư NĐ169 + license Platts/SGGO — chưa xác nhận xong.
