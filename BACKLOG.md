...
- **Cập nhật lớn (2026-09-11→15): 28/30 VN30 đã có B01 FY2024, và B02 (KQKD) đã bắt đầu triển khai** (trước đó B01-only). Phạm vi B02 hiện chỉ cho scheme `TT200_DN` (doanh nghiệp thường) — ngân hàng/bảo hiểm/CTCK có cấu trúc KQKD khác hẳn, chưa có trong KB, `IS_SCHEME_FOR_BS_SCHEME` cố tình không map để không đoán sai. Còn thiếu FY2024: **TCB** (không có ở mirror vietstock; tìm được PDF IR riêng nhưng trình bày hoàn toàn tiếng Anh, không dùng mã chỉ tiêu Việt Nam — không tương thích dict hiện tại, cần scheme tiếng Anh riêng nếu muốn tự động hoá) và **SSI** (không có ở mirror, chưa dò nguồn khác).
- **3 lỗi hệ thống mới trong crawler tìm và sửa tận gốc trong đợt này** (không phải vá riêng từng công ty):
  1. Dấu gạch/dấu tiếng Việt trong marker "B01-DN/HN" bị OCR đọc sai ở DPI thấp (VD: gạch ngang → dấu ngã) → chuẩn hoá bỏ dấu tập trung 1 chỗ (`_strip_diacritics` trong `ocr_text()`), mọi regex chỉ cần khớp bản ASIIC.
  2. Trang mục lục & lời ý kiến kiểm toán nhắc tên các báo cáo trong văn xuôi → bị nhận nhầm là trang bắt đầu → thêm ngưỡng "mật độ số tiền" (`MIN_MONEY_MATCHES`) để loại trang không phải bảng số liệu thật.
  3. Câu chú thích chuẩn cuối mọi trang "...là phần không thể tách rời của báo cáo này" chứa cụm từ trùng với tiêu đề phần thuyết minh → cắt cụt bảng cân đối giữa chừng (case NVL FY2024, mất 3/4 trang) → phân biệt bằng ngữ cảnh xung quanh cụm khớp.
  4. **Phát hiện quy ước trình bày B02 khác nhau giữa các công ty**: GAS/HPG in chi phí dương (công thức tự trừ), PDR in chi phí âm/ngoặc đơn (công thức phải cộng) — validator giờ tự nhận diện qua dấu mã 11 (Giá vốn hàng bán) và đảo trọng số phù hợp, không giả định 1 quy ước cứng.
- **2026-09-15 (cùng ngày, tiếp)**: user chỉ ra KB ngành đặc thù (`nganhang_chungkhoan_baohiem_ma_ke_toan_bctc.md`) đã tồn tại — đúng cho mã tài khoản + bảng cân đối, nhưng **KQKD (P&L) 3 ngành thật sự chưa có**. Đã research + verify bằng dữ liệu thật (VCB/BVH/VIX) và **test crawl tự động thành công cả 3** (0 vi phạm sau khi sửa vài mã đọc sai — xem KB §16.5-16.8): ngân hàng dùng mẫu B03/TCTD-HN (STT La Mã, mã tự gán `PL.*`), bảo hiểm holding dùng khối mã 01-48 riêng trong B02-DN/HN, CTCK dùng B02-CTCK (mã số thật). **3 ngành dùng 3 quy ước dấu khác nhau** — đã hard-code riêng từng scheme trong `IS_FORMULAS`. B02 giờ chạy được cho **cả 4 nhóm ngành** (trước chỉ TT200_DN), nhưng mỗi ngành mới verify đúng 1 công ty — cần thêm mẫu trước khi tin tưởng áp dụng hàng loạt.
- **2026-09-15 (cùng ngày, tiếp nữa) — SSI xong, TCB vẫn chặn**:
  - **SSI**: tìm được PDF IR thật (`ssi.com.vn`, trước đó 403 vì thiếu `User-Agent` — đã sửa `download_pdf()`, không phải anti-bot bypass, chỉ giả browser thường như `curl` mặc định đã làm). B01+B02 (CTCK_B01/CTCK_B02, hợp nhất) test lần đầu ra 9 vi phạm ở B02 — điều tra bằng OCR 300dpi đối chứng phát hiện: (1) **mã "80" nghĩa khác nhau giữa VIX và SSI** (VIX: "Cộng chi phí tài chính"; SSI: "Cộng kết quả hoạt động khác" — 2 khái niệm khác hẳn cùng số mã) — đã sửa `IS_FORMULAS['CTCK_B02']` bắt buộc dùng mã "60" thay vì "80" cho target 70, công ty theo quy ước VIX (không có mã 60) giờ báo INFO_NULL thay vì validate nhầm số; (2) hàng loạt lỗi đọc số thật của Gemini (mã 09/10/21.x/24/26-32/42/44/52/55/70/91/200 — có cả case mã bị dịch lệch 1 vị trí giữa các dòng phụ, ví dụ giá trị đúng của mã 23 (=dash/NULL năm nay) bị gán nhầm sang chỗ khác) — sửa bằng xoá sạch + chèn lại toàn bộ 53 dòng B02 từ OCR 300dpi đối chứng tay từng số. **Kết quả cuối: SSI 0 vi phạm** (2 INFO_NULL đúng ý nghĩa cho mã 23/202 — dash thật trong nguồn).
  - **TCB — vẫn CHƯA xong, lý do mới xác định rõ**: URL IR đang dùng (`techcombank.com/.../techcombank-vas-bao-cao-tai-chinh-hop-nhat-4q24-searchable.pdf`) là **báo cáo QUÝ (Quý IV 2024), không phải BCTC năm đã kiểm toán** — cấu trúc khác hẳn những gì pipeline giả định: đơn vị tính **triệu đồng** (không phải đồng), cột "Quý IV 2024/2023" + "Luỹ kế từ đầu năm" (không phải "Năm nay/Năm trước"), thứ tự thống kê trong file đảo (báo cáo KQKD mẫu B03a/TCTD-HN xuất hiện ở trang 8, trước cả bảng cân đối — nên scan tìm B01 bắt nhầm nội dung trang KQKD, gán sai scheme `CTCK_B01` do marker "TCTD" không khớp `BANK_B02_TCTD_HN`'s pattern cứng cho "B02/TCTD" trong khi văn bản thật ghi "B03a/TCTD-HN"). **Đây không phải bug parse — là sai loại tài liệu.** Cần tìm đúng BCTC NĂM đã kiểm toán của TCB (không phải bản quý) trước khi thử lại; chưa dò nguồn thay thế trong session này để tiết kiệm hạn ngạch Gemini (20 request/ngày, đã dùng cho SSI).
- **Gemini free tier: hạn ngạch CỨNG 20 request/ngày/model** (`GenerateRequestsPerDayPerProjectPerModel-FreeTier`, xác nhận qua response lỗi 429 trực tiếp) — không phải rate-limit theo phút như tưởng trước đó. Với B01+B02 = 2 request/công ty, **tối đa ~10 công ty/ngày** ở tốc độ hiện tại. Đây là con số cụ thể để tính thời gian hoàn tất 30 công ty × nhiều năm — củng cố thêm quyết định chuyển sang Kaggle/model nguồn mở đã ghi ở trên (vẫn CHƯA triển khai, chỉ mới quyết định hướng).
...
- ✅ **2026-08-31→09-14 (trước session này)**: `crawl_tools/data_quality_check.py` overhaul — sửa 2 `dup_key` sai gây false ERROR, mọi issue giờ in ra stdout (không chỉ nằm trong email), thêm `reconcile_sjc.py` (đối chiếu ngược với webgia.com — bắt được cả lỗi "ghi rồi" lẫn "chưa từng ghi", việc mà check pre-insert không bao giờ thấy được). Xem CLAUDE.md mục "Retrospective reconciliation".
- ✅ **2026-09-15 (session này)**: phần cuối cùng của yêu cầu gốc — `sys.exit()` giờ đỏ khi có **ERROR, không chỉ CRITICAL** (2 chỗ: nhánh SMTP chưa cấu hình + nhánh cuối cùng), đúng như mục này yêu cầu từ đầu.
- **Chạy thật ngay sau khi sửa (2026-09-15)**: `0 CRITICAL / 0 ERROR / 4 WARNING` — toàn bộ WARNING đều là gap đã biết/đã tài liệu hoá (SBV rate không có row từ 09-11, vn30 pe/pb 81% NULL do outage đã biết, 1 giai đoạn giá vàng đứng yên 36 ngày hồi 2024-06 chưa backfill).
- ⏳ **Còn lại, cần user (không tự làm được)**: gửi email đang lỗi `535 Username and Password not accepted` — **Gmail app password cho `SMTP_USER` cần cấp lại** (Google Account → App Passwords) rồi cập nhật secret `SMTP_PASS` trên GitHub repo. Tới lúc đó `data-quality-check.yml` sẽ đỏ mỗi ngày vì lỗi gửi mail (đây là hành vi ĐÚNG theo thiết kế mới — lỗi gửi báo cáo cũng là một sự cố thật) chứ không phải vì lỗi dữ liệu.
- **Tại sao:** cảnh báo không ai đọc = không có cảnh báo.

### [DQ-02] Lỗ hổng dữ liệu GSO còn tồn đọng — ✅ phần lớn xong (backfill đã chạy)
- Tình trạng cũ (khi mở mục này): `vn_gso_gdp_quarterly`/`vn_gso_iip_monthly`/`vn_gso_trade_monthly` có 0 rows — bảng tồn tại nhưng chưa từng có dữ liệu.
- ✅ **Đã backfill (2026-09, trước session này) — xác nhận lại số liệu thật 2026-09-15**: `vn_gso_gdp_quarterly` 86 rows, `vn_gso_iip_monthly` 65 rows, `vn_gso_trade_monthly` 77 rows, `vn_gso_cpi_monthly` 294 rows (2002-01 → 2026-08). Cả 4 crawler giờ có `crawl_period(year, month_or_quarter)` để target đúng kỳ thay vì chỉ lấy bản tin mới nhất — xem CLAUDE.md mục "GSO GDP/IIP/Trade crawlers".
- **Gap còn lại là gap PHÍA NGUỒN, đã xác minh không phải bug**: IIP thiếu 16 tháng cuối-quý (NSO công bố "giá trị tăng thêm" thay vì IIP thật cho tháng đó); GDP thiếu `2024Q1`; Trade thiếu `2020-12`/`2024-03`/`2026-05` (không tìm thấy bản tin đúng kỳ trong cửa sổ tìm kiếm, kể cả nới rộng). Xem CLAUDE.md để biết chi tiết từng kỳ — **không backfill bằng cách đoán/LLM**, để `null` đúng như source.
- `global_macro`: `sp500`/`dowjones` NULL 31 ngày/năm (chủ yếu cuối tuần/lễ) — public đúng là `null`, không còn `0`.
- **Tại sao còn 🟡 không phải ✅ hẳn:** chưa re-verify `vn_gso_cpi_monthly`'s `cpi_yoy_pct` NULL count (17 tháng, số liệu cũ) trong session này — nếu vẫn còn thiếu, đây là gap độc lập với backfill GDP/IIP/Trade ở trên, cần review riêng lần tới.
...
- **Đã dịch đầy đủ 7 trang dev/agent-builder-facing** (đúng phạm vi đã chốt, loại legal ra vì rủi ro dịch sai pháp lý chưa qua luật sư): `docs.html` (20 key), `api-docs.html` (43 key, kèm cả `GROUP_LABELS`/text JS-generated của endpoint catalog fetch động — nghe event `docs-lang-changed` để re-render đúng ngôn ngữ), `guide-buyer.html` (50 key), `guide-seller.html` (45 key), `knowledge-pack-spec.html` (65 key, gồm cả 2 block skeleton + 2 ví dụ dài dịch nguyên khối), `google-sheets.html` (53 key), `excel.html` (40 key) — **cả hai dictionary EN này đã bị xoá 2026-09-25**: `excel.html` không còn tồn tại, còn `google-sheets.html` được viết lại hoàn toàn nên bản dịch cũ sai nội dung. Trang Sheets hiện chỉ có tiếng Việt, không hiện nút EN/VI. Mỗi file đã verify: số key trong HTML khớp 100% số key trong dictionary (không thiếu/thừa), HTML parse sạch, JS syntax hợp lệ.
...
- [ ] **Fuel-forecast B2B (dự báo giá xăng dầu trong nước) — Phase 1, Plan 1 (data pipeline) đang triển khai** trên branch `feat/fuel-forecast-pipeline`. Kiến trúc medallion (Bronze R2 → Silver `FUEL_FORECAST_DB`). **Plan 1 + Plan 2 LIVE** (merged PR #1 + Plan 2 on main, 2026-07-12). 36 unit test xanh (giảm từ 75 sau khi xoá structural-v1, xem dưới). **Dữ liệu (2026-09-10):** 25 kỳ BCT/fuel (đến 2026-08-27), Bronze R2 raw + sha256.
  ...
  - [ ] **QUYẾT ĐỊNH SẢN PHẨM ĐANG CHỜ (2026-09-20): VPI đã làm việc này tốt hơn ta 7–10 lần, miễn phí.** Viện Dầu khí Việt Nam (thuộc Petrovietnam) công bố dự báo giá xăng dầu trước **mỗi** kỳ điều hành, cho cả 5 loại, bằng mạng nơ-ron, phát qua VietnamPlus/BNews/PetroTimes/HTV/PVOIL. Đo trên chính `fuel_price_cycle`: **VPI MAE 70 đ/l (E5RON92, n=4) và 62 đ/l (DO005S, n=2)**, so với 513/600 của mô hình dự báo-trước-kỳ của ta và 199/236 của `delta-world-v1` (vốn đã biết đáp án). Họ thắng vì **mô hình hoá luôn hành động của Quỹ bình ổn** — đúng nguồn sai số lớn nhất ta không xử lý được. → **Không định vị sản phẩm là "bán dự báo giá xăng"** nữa, kể cả `delta-world-v1` đang bán. Hai hướng còn sống vì VPI không có: (a) kho lịch sử sạch có API, (b) **chấm điểm dự báo so với thực tế** — lưu dự báo VPI mỗi kỳ rồi công bố độ chính xác theo thời gian. Chi tiết: `CLAUDE.md` mục "Fuel price forecasting is a solved market — VPI".
  - [ ] **Lấy dữ liệu VPI — chưa có đường sạch.** Kỹ thuật: `vpi.pvn.vn` redirect sang trang Gamma, **không API/RSS/kho dữ liệu**, chỉ có `contact@vpi.pvn.vn`; bài báo đọc và parse được (bnews 200, regex lấy đúng số), lead time **25–45 giờ trước kỳ** (đo 4 bài) nên kịp dùng — nhưng `/rss/kinh-te.rss` lỗi 500 và URL bài có id số không đoán được, nên phải dò bằng search/category: **đúng kiểu đã làm crawler MOIT chết 2 lần trong 2026**, lần này lại phụ thuộc CMS bên thứ ba. Pháp lý 3 tầng: con số nhiều khả năng dùng được (Điều 15 Luật SHTT loại "tin tức thời sự thuần tuý"), câu chữ bài báo thì không, còn **tập hợp có hệ thống trong sản phẩm thu tiền** nhiều khả năng rơi vào diện phải có giấy phép trang thông tin điện tử tổng hợp (NĐ 72/2013) — VDV chưa có. **Việc kế tiếp rẻ nhất: gửi thư hỏi VPI cấp phép dữ liệu.** Chưa làm.
  - [ ] **Thu thập 20–30 kỳ dự báo VPI để kiểm định chắc chắn** trước khi chốt quyết định trên — n=4/n=2 hiện tại là nhỏ, và báo chí có thể chỉ đăng kỳ VPI đoán trúng. Lưu ý việc này **tự nó đã là cào có hệ thống**, nên làm sau khi có câu trả lời pháp lý.
  - [ ] **`fuel_price_cycle` thiếu kỳ và có kỳ đáng ngờ (phát hiện 2026-09-19/20).** (a) Hai kỳ **có thật nhưng thiếu trong DB**: `2025-07-03` và `2026-01-22` (biết được vì VPI có dự báo cho chúng). (b) **6 cặp kỳ cách nhau ≤3 ngày**, đều là bản tin riêng URL khác nhau: 12/13-06-2023, 20/21-06-2023, 08/09-01-2025, 05/08-05-2025, 07/10-03-2026, 25/26-03-2026. (c) Cụm tháng 3/2026 cho hệ số truyền dẫn ngầm định **600, 441 và −288** với E5RON92, so với 131–172 ổn định ở phần còn lại; DO005S cùng kỳ vẫn bình thường (208, 172). **`CLAUDE.md` đang giải thích kỳ 26/03/2026 là do Quỹ bình ổn can thiệp** — nhưng 25/03 và 26/03 cách nhau đúng 1 ngày, nên lỗi dữ liệu đáng cân nhắc ngang giả thuyết chính sách, và **chưa ai đối chiếu với HTML gốc trong R2**. Đây cũng chính là 2 kỳ được cho là kéo R² của xăng từ 0,91 xuống 0,65 → nếu là lỗi parse thì cả kết luận "cần thêm biến can thiệp BOG" cũng sai nền.
  - **Audit toàn bộ `FUEL_FORECAST_DB` (2026-09-19)** — bảng chi tiết trong `CLAUDE.md` mục "`FUEL_FORECAST_DB` — what each table is for". Kết quả: chỉ `fuel_price_cycle` (234 dòng = 117 kỳ × 2 loại) nuôi model, và chỉ qua 2 cột `world_avg_price` + `retail_price`; `fuel_forecast` (552 dòng) là thứ API bán. `fuel_formula_params` **đã DROP** — 0 dòng từ ngày tạo, không code nào đọc/ghi, vốn là schema của `structural-v1` đã xoá. **`fuel_world_daily` (Brent/RBOB) không còn consumer nào** — vẫn để crawler chạy (miễn phí, không rủi ro pháp lý) làm dữ liệu tham khảo, không tự động nối lại vào model nếu không có lý do rõ ràng (chính là proxy đã thất bại).
  - [ ] **`fuel_backtest` đang write-only — số liệu độ chính xác trên FE là hằng số hardcode.** Pipeline refit bảng này mỗi kỳ (70 dòng MAE/RMSE/coverage/skill) nhưng không endpoint nào query, còn `fe/pages/fuel-forecast.html` ghi cứng `skillByFuel`/`r2ByFuel` trong JS. Đã lệch thật: trang hiện skill 0.745/0.778, refit 19/09 cho 0.741/0.765. Cần hoặc serve từ `fuel_backtest`, hoặc bỏ cách trình bày như thể là validation sống — **không sửa bằng cách gõ lại hằng số**. Liên quan: nhãn "Skill vs random-walk" chưa nói rõ đây là đo ex-post (đã biết Δworld), xem mục dưới.
...
- [x] ~~Excel Add-in — manifest.xml + task pane, mount /excel-addin/, workbook `.xlsx` từ `/api/v1/excel/workbook`~~ — **đã gỡ bỏ hoàn toàn 2026-09-25** (UI + source + CORS Office). Sản phẩm rút còn 2 luồng: API và Google Sheets. Chỉ `/api/v1/excel/refresh-data` được giữ lại vì template Sheets gọi nó.
...
- [x] **2026-09-14: Lớp guard THỨ BA — crawl chuyển hẳn về server riêng, bỏ GitHub khỏi đường crawl.** User báo chart vẫn lệch (chart 142,7 — nguồn 143,2). Điều tra ra 3 sự thật:
  1. **Hôm 14/09 GitHub Actions không chạy lần nào** (bỏ sạch slot đã khai báo), và lần chạy gần nhất 13/09 **FAILED** vì runner **timeout khi kết nối `www.24h.com.vn`** — cùng loại vấn đề với `nso.gov.vn` chặn IP datacenter nước ngoài, chính lý do prod đã dời về VPS Việt Nam hồi 04/09.
  2. **Thứ đang thực sự ghi DB là systemd timer trên box** (`crawl_time=01:45:25`/`01:45:32` UTC khớp chính xác `OnCalendar=01:45 UTC`; bạc cũng cùng lần chạy đó) — trong khi **cả `CLAUDE.md` lẫn `DEPLOY.md` đều khẳng định timer này "INACTIVE, chưa từng dựng lại trên box mới"**. Tài liệu sai, và cái sai đó làm tôi tìm nhầm chỗ mất cả ngày.
  3. **`crawl-fallback.sh` có y hệt guard "hôm nay có row rồi thì bỏ qua"** (dòng 100) — tức là lớp thứ ba của cùng một lỗi, và là lớp **duy nhất đang chạy thật**. Sửa 2 lớp kia hôm 12–13/09 nên với người dùng chưa đổi gì cả.
  - **Đã làm:** box thành đường crawl chính — bỏ guard (mọi lần chạy đều upsert), timer đổi từ 2 mốc sang **5 mốc 2 tiếng/lần giờ hành chính** (01:00–09:00 UTC = 08:00–16:00 VN), và mỗi lần chạy thành công **tự sinh lại `fe/data/*.json`** vào đúng thư mục FastAPI phục vụ (chart đọc file tĩnh chứ không đọc DB, nếu không thì dòng mới nằm im tới 6h). `gold-silver-crawl.yml` gỡ hết schedule, chỉ còn `workflow_dispatch` để bấm tay khi box chết → chỉ còn đúng 1 nơi ghi.
  - **Đã sửa tài liệu sai:** mục box-fallback trong `CLAUDE.md` + mục 4 `DEPLOY.md` (từ "INACTIVE" → "PRIMARY", kèm câu "đừng tin file này hơn box, chạy `systemctl list-timers` là rõ").
  - **Cần thao tác tay trên box** (deploy chỉ đưa file xuống, không nạp lại systemd): `cp deploy/crawl-fallback.{service,timer} /etc/systemd/system/ && systemctl daemon-reload && systemctl enable --now crawl-fallback.timer`.
  - **Bài học lặp lại lần 2:** hôm 12/09 đã tự ghi "phải rà toàn bộ chuỗi gọi, guard trùng ý tưởng có thể nằm ở nhiều tầng" — vẫn sót lớp 3, vì tin tài liệu nói nó không chạy thay vì kiểm chứng bằng dữ liệu. Lần sau: xác định **ai đang thực sự ghi** (đọc `crawl_time`) trước khi sửa bất kỳ lớp nào.
- [x] **2026-09-13: Hệ quả của bug trên — chuyển vàng về CHỈ còn SJC + sửa 14 ngày dữ liệu sai + đổi lịch 2 tiếng/lần.** Bug freeze sống 75 ngày (29/6 do `d31423068` → 12/9) nên *mọi* dòng vàng trong khoảng đó là giá sáng bị đóng băng, không riêng ngày nào.
  ...
  - ~~**Lịch mới:** 2 tiếng/lần trong giờ hành chính (`'7 1,3,5,7,9 * * *'`)~~ — **đã thay 14/9**: 1 tiếng/lần, chạy bằng systemd timer trên box, không còn dùng GitHub cron (xem mục 2026-09-14 bên dưới).
  - ~~**Giữ nguyên có chủ đích:** crawler vẫn parse + validate TẤT CẢ brand rồi mới lọc SJC vì `validate_gold_records` cần ≥3 brand.~~ — **đã thay 14/9**: `crawl_sjc_24h.py` chỉ đọc đúng dòng SJC và validate qua `sjc_store.py`. `validate_gold_records` hiện không còn caller production nào. Đổi như vậy là *tăng* độ phủ, không giảm: lỗi rớt chữ số bị chặn bởi sàn 20M của `check_plausible`, còn cross-brand median thì **đảo chiều khi dữ liệu rác chiếm đa số** (trên các ngày 2015 nó flag đúng dòng BTMC/DOJI *đúng*), trong khi so chéo 2 nguồn không có lỗ hổng đó.
...
- [x] **2026-09-14: Hai luồng crawl SJC độc lập + siết lại tiêu chí "chạy thành công".** Sau khi dọn DB chỉ còn SJC, dựng lại pipeline từ đầu.
  - **2 crawler, 2 nguồn không liên quan nhau:** `crawl_sjc_24h.py` (24h.com.vn, nguồn hiển thị) và `crawl_sjc_giavang.py` (giavang.org). Cả hai ghi qua `crawl_tools/sjc_store.py` — một chỗ duy nhất giữ write path + luật hợp lệ + so chéo. `sjc.com.vn` (chính chủ) trả **403** cho request từ server nên không dùng được.
  - **Migration 016** đổi unique key `(date, type)` → `(date, type, source)`. Giữ key cũ thì 2 nguồn ghi đè nhau và mọi bất đồng đều vô hình.
  - **Read path *chọn* nguồn, không lấy "ai ghi sau"**: `generate_static_data.py` và `market_data.py` cùng `ORDER BY CASE source` (24h trước, giavang sau). **Sửa một chỗ phải sửa cả hai.**
  - **Lịch:** 1 tiếng/lần, 9 lượt phủ 8 tiếng hành chính (01:00–09:00 UTC = 08:00–16:00 VN), systemd timer trên box. Không còn schedule trên GitHub Actions — runner ngoài VN timeout khi gọi 24h.com.vn.
  - **Tiêu chí thành công đổi từ "hôm nay có dòng chưa" sang "`crawl_time` có nhích lên không"**, đo riêng từng writer. Kiểu cũ báo xanh cả buổi chiều dù nguồn đã chết từ 08:00 — đúng cái lỗi mà nguồn thứ 2 sinh ra để phát hiện. So `before`/`after` của cùng một query nên lệch múi giờ box/container không giả được độ tươi. Exit code của 2 crawler SJC (`sjc_rc`) tách khỏi `crawl_gold_silver.py` (`crawl_rc`) vì Yahoo chặn IP datacenter → nó đỏ mỗi ngày và sẽ nuốt mất lỗi thật.
  ...
  - **Đã bịt lỗ hổng đối chiếu hồi tố** (`crawl_tools/reconcile_sjc.py`): đọc lại lịch sử SJC công bố từ `webgia.com` — nguồn thứ 3, độc lập, và **phục vụ được ngày quá khứ** (24h/giavang chỉ biết hôm nay). Bắt buộc trang phải tự khai ngày của nó (`<h1>`) trước khi tin — đúng bài học của 196 dòng giả. 5 verdict, **chỉ `fabricated` là ERROR**; `carry_forward` và `unconfirmed` ban đầu đều là ERROR báo nhầm trên dữ liệu thật. **Không bao giờ tự sửa dữ liệu** — nguồn thứ 3 lệch là bằng chứng để xem, không phải chân lý để ghi đè. Kèm `structural_audit()` không cần mạng: dòng có `crawl_time` giả, giá đứng yên 20+ ngày, và outlier **theo từng thời kỳ** (so với median cửa sổ ±45 ngày của chính nó, vì ngưỡng 20M–500M cố định không phân biệt được 81M năm 2015 với 81M năm 2024).
  - **Chạy lần đầu đã tìm ra thiệt hại còn sót:** khoảng 29/6–12/8 (phần freeze mà đợt sửa 13/9 **chưa đụng tới** — đợt đó chỉ làm từ 13/8) có 16 ngày `stale`, 1 `carry_forward`, 5 `unconfirmed`, 1 `fabricated`.
  - **[x] Đã sửa 15/9/2026**: 16 ngày có bằng chứng chắc (15 `stale` + 1 `fabricated`, đều lấy giá đóng cửa thật từ webgia.com) — `be/migrations/fix_gold_freeze_stale_days.py --apply`, backup CSV kèm theo. **Cố tình không đụng**: `2026-08-07` (`carry_forward`, đã đúng sẵn) và 5 ngày `unconfirmed` (08–09/07, 15–17/07 — archive không có mục, vắng bằng chứng ≠ bằng chứng sai, không đủ căn cứ để sửa). Chạy lại `reconcile_sjc.py` xác nhận 0 ERROR / 0 stale sau khi sửa.
  - **Còn tồn:** lịch sử SJC thưa: 5 dòng 2015, 11 dòng 2016, trống 2017–2023.
