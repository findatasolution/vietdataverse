...
- [ ] **Fuel-forecast B2B (dự báo giá xăng dầu trong nước) — Phase 1, Plan 1 (data pipeline) đang triển khai** trên branch `feat/fuel-forecast-pipeline`. Kiến trúc medallion (Bronze R2 → Silver `FUEL_FORECAST_DB`), model hybrid NĐ80, feature MRI, corporate API riêng OAuth2 + `.corp-prod-env`. **Plan 1 + Plan 2 LIVE** (merged PR #1 + Plan 2 on main, 2026-07-12). 75 unit test xanh. **Dữ liệu:** 23 kỳ BCT (backfill archive), Brent/RBOB 2 năm (1.004 rows), Bronze R2 raw + sha256. **Model (đột phá):** level model từ proxy Brent/RBOB **thua** random-walk (skill −0.5…−1.2); **delta pass-through trên giá MOPS đúng loại thắng RW cả 3 fuel**: `retail_t ≈ retail_{t−1} + k·Δworld`, k≈122–153 VND/L/USD-bbl (BOG/thuế hấp thụ 20–40%). **Walk-forward (n=15–16): MAE DO 488 / E5 651 / RON95 810 VND/L, coverage 87–94%, skill +0.45…+0.79** — con số bán B2B, ghi trong `fuel_backtest`. CI weekly `fuel-pipeline.yml` (extract-land ▶ model) đã xanh với đủ secrets. **Business case đã lượng hoá:** proxy futures free KHÔNG đủ → license feed Platts/MOPS daily = chuyển skill âm thành dương → mục chi license là bắt buộc để bán forecast pre-announcement; không có feed thì bán scenario world-conditional. **Kế tiếp:** Plan 3 (public API giá lịch sử), Plan 4 (corporate API service + `.corp-prod-env`). **Trước khi thu tiền:** ý kiến luật sư NĐ169 + license Platts/SGGO.
