...
- [x] **2026-09-12: Chart giá vàng/bạc đứng im ở giá mở cửa cả ngày dù giá nguồn đổi nhiều lần trong ngày** — báo cáo trực tiếp: chiều 12/9 nguồn (24h.com.vn) đã cập nhật DOJI HN lên 143,0/146,0 triệu nhưng chart vẫn hiện 142,4/145,4 triệu (giá crawl lúc 08:45 sáng). Nguyên nhân: `crawl_gold_silver.py` có guard "nếu hôm nay đã có row thì skip", nên 8 lần rerun trong ngày (9h07–16h07 VN, vốn có sẵn để retry khi crawl sáng lỗi) chưa từng thực sự ghi đè giá — chốt cứng giá mở cửa cả ngày dù `ON CONFLICT DO UPDATE` đã có sẵn nhưng không bao giờ được chạm tới. Đã sửa cả 3 chỗ (gold + 2 nguồn silver) bỏ hẳn guard, luôn upsert mỗi lần crawl thành công — comment giải thích trong `crawl_gold_silver.py` + cập nhật comment lịch cron trong `gold-silver-crawl.yml`. Đã chạy crawler thủ công + trigger tay workflow `Generate Static Chart Data` để vá ngay dữ liệu hôm 12/9 thay vì chờ vòng chạy tiếp theo. **Tại sao đây là bug thật, không phải chỉ "độ trễ 1 lần/ngày":** hệ thống retry hourly vốn đã tồn tại sẵn cho mục đích khác (retry khi lỗi) — sửa lỗi này khiến retry cũng đồng thời làm việc đúng chức năng thứ hai mà thiết kế ban đầu vô tình chặn mất.
